...
def user_login_task(username):
	try:
		...
		if check_user.exists() and check_user.filter(is_active=True).exists():
			...
			if send_active_mail(check_user.values()[0]['email']):
				return 2
			else:
				return 4
		else:
...
def user_forget_task(email, active_code):
	try:
		redis.set(email, active_code)
		redis.expire(email, 120)
		body = 'http://localhost:8000/accounts/change-password/{}/{}/'.format(signer.sign(email),
		                                                                      signer.sign(active_code))
		send_mail('فراموشی رمز عبور', body, settings.EMAIL_HOST_USER, [email])
		return 1
	except User.DoesNotExist:
		return 4